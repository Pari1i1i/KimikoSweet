      ...
      <div className="max-w-6xl mx-auto flex items-center justify-between gap-1.5 sm:gap-2">
        ...
        <nav className="flex items-center gap-1 sm:gap-2 shrink-0">
          <button
            ...
            className={`px-2 sm:px-3 py-1.5 rounded-neo-sm text-[11px] sm:text-sm font-bold border-2 border-brand-dark transition-all ${
              activeTab === "menu"
          ...
          <button
            ...
            className="relative px-2 sm:px-3.5 py-1.5 rounded-neo-sm bg-brand-pink text-brand-dark font-bold text-[11px] sm:text-sm border-2 border-brand-dark neo-btn-sm flex items-center gap-1 cursor-pointer"
            aria-label="Keranjang Belanja"
          >
            <ShoppingBag className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
            <span className="hidden sm:inline">Keranjang</span>
            {totalItems > 0 && (
              <span className="bg-brand-accent text-white text-[9px] sm:text-[11px] font-extrabold px-1.5 py-0.2 rounded-full border border-brand-dark shadow-[1px_1px_0px_#1A1A1A] animate-pulse">
                {totalItems}
              </span>
            )}
          </button>

          {/* Admin link / Tombol Dapur Penjual (Jelas & Mudah Ditekan di HP) */}
          <Link
            href="/admin"
            className="h-8 sm:h-9 px-2 sm:px-2.5 rounded-neo-sm bg-brand-cream border-2 border-brand-dark hover:bg-brand-butter neo-btn-sm flex items-center justify-center gap-1 cursor-pointer select-none active:scale-95 transition-transform"
            title="Dapur Penjual (Admin Login)"
            aria-label="Dapur Penjual"
          >
            <ChefHat className="w-4 h-4 text-brand-dark shrink-0" />
            <span className="text-[10px] font-bold text-brand-dark hidden md:inline">Admin</span>
          </Link>
        </nav>
      </div>
    </header>
